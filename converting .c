...
void converting() {
	...
	printf("*** Start of Converting Strings to long Demo ***\n");		//Prints intro of application of V3
	char   longString[BUFFER_SIZE];									    // Stores long typed input from user here with max length equaling buffer size set
	long   longNumber;											    	//Stores converted long number/output of converting function here
	do {
		printf("Type the long numeric string (q - to quit):\n");        //Requests long numeric string from user
		fgets(longString, BUFFER_SIZE, stdin);						    //Receives and stores information presented to function such as input and buffer size set
		longString[strlen(longString) - 1] = '\0';					    //Sets null terminator
		if ((strcmp(longString, "q") != 0)) {							//Checks if input was "q" 
			longNumber = atol(longString);							    //Converts character string entered into long value
			printf("Converted number is %ld\n", longNumber);			    //Prints long numeric value
		}
	} while (strcmp(longString, "q") != 0);							    //V3 of Program will loop until Input 'q' is given to quit the program
	printf("*** End of Converting Strings to long Demo ***\n\n");		//Prints conclusion of V3 application
}